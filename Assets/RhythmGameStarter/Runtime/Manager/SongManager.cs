        ...
        void Update()
        {
            ...
            if (!songPaused && songHasStarted)
            {
                ...
                if (songPosition >= 0)
                {
                    ...
                    {
                        if (songPosition < currentSongItem.clip.length)
                        {
                            var now = new DateTime((long)((currentSongItem.clip.length * TimeSpan.TicksPerSecond) - (songPosition * TimeSpan.TicksPerSecond)));
                            onSongProgressDisplay.Invoke(now.ToString("mm:ss"));
                            GameManager.Instance.txt_Time.text = now.ToString("mm:ss");
                        }
                    }
                }
            }
