...
namespace RhythmGameStarter
{
    public class NoteArea : MonoBehaviour
    {
        ...
        [NonSerialized]
        public BaseInputHandler keyboardInputHandler;

        [NonSerialized]
        public BaseTouchHandler touchInputHandler;
        ...
        void Update()
        {
            ...
            if (NoteRecorder.INSTANCE && NoteRecorder.INSTANCE.IsRecording)
            {
                ...
                if (hasTriggeredSomething && !hasPossibleDirection)
                {
                    ...
                    if ((keyboardInputHandler && keyboardInputHandler.GetTrackDirectionKey(Note.SwipeDirection.Left)) || diff.x >= NoteRecorder.INSTANCE.swipeThreshold)
                    {
                        possibleDirection = Note.SwipeDirection.Left;
                        hasPossibleDirection = true;
                    }
                    else if ((keyboardInputHandler && keyboardInputHandler.GetTrackDirectionKey(Note.SwipeDirection.Right)) || diff.x <= -NoteRecorder.INSTANCE.swipeThreshold)
                    {
                        possibleDirection = Note.SwipeDirection.Right;
                        hasPossibleDirection = true;
                    }
                    else if ((keyboardInputHandler && keyboardInputHandler.GetTrackDirectionKey(Note.SwipeDirection.Up)) || diff.y <= -NoteRecorder.INSTANCE.swipeThreshold)
                    {
                        possibleDirection = Note.SwipeDirection.Up;
                        hasPossibleDirection = true;
                    }
                    else if ((keyboardInputHandler && keyboardInputHandler.GetTrackDirectionKey(Note.SwipeDirection.Down)) || diff.y >= NoteRecorder.INSTANCE.swipeThreshold)
                    {
                ...
                if ((currentFingerID != -1 && (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled)) || (keyboardInputHandler && keyboardInputHandler.GetTrackActionKeyUp(track, track.transform.GetSiblingIndex())))
                {
            ...
            if (currentNote || (currentNote && keyboardInputHandler))
            {
                switch (currentNote.action)
                {
                    case Note.NoteAction.LongPress:
                        ...
                        else if (touch.phase == TouchPhase.Ended || touch.phase == TouchPhase.Canceled || (keyboardInputHandler && keyboardInputHandler.GetTrackActionKeyUp(track, track.transform.GetSiblingIndex())))
                        {
        ...
        public void TriggerNote(TouchWrapper touch)
        {
            ...
            Note notes = note as Note;

            if (note)
            {
                if (!notes.m_NoteSetup.m_PlayerNote)
                {
                    GameManager.Instance.UpdateVsBar(false);
                    // SoundManager.Instance.PlaySoundCrack();
                    EventManager.CallEvent(GameEvent.VIBRATE_HEAVY);
                    // GameManager.Instance.m_NoteInGame.Remove(notes);
                    return;
                }
                // note.m_NoteSetup.AnimateNote();
                ...
                switch (note.action)
                {
                    ...
                    case Note.NoteAction.LongPress:
                        // if (touch.phase == TouchPhase.Began)
                        // {
                        //     note.m_NoteSetup.sr_LongLine.material.SetFloat("_Glow", 7);
                        //     Helper.DebugLog("ZZZZZZZZZZZZZZZ");
                        // }
                        if (touch.phase != TouchPhase.Began) break;
            ...
            else
            {
                ...
                int curScore = StatsSystem.Instance.score;
                if (curScore >= 10)
                {
                    StatsSystem.Instance.score -= 10;
        ...
        private void AddCombo(Note note, Vector3 touchDownPosition)
        {
            ...
            songManager.comboSystem.AddCombo(note, 1, Mathf.Abs(diff), note.score);
